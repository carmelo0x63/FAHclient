...
LABEL maintainer="carmelo.califano@gmail.com"
...
RUN apt update -q && apt install --no-install-recommends -yq curl bzip2 ca-certificates && \
...
CMD ["--user=Anonymous", "--team=0", "--gpu=false", "--smp=true", "--power=light"]
